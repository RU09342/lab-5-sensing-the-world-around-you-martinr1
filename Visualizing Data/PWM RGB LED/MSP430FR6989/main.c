...
int main(void)
{
    ...
    P3SEL0 |= BIT3; // red RGB
    P2DIR |= BIT0 + BIT1 + BIT6 + BIT7; // two GPIO LEDs
    P2OUT |= BIT0 + BIT1;
  ...
  //ADC12MCTL1 |= ADC12INCH_4 | ADC12VRSEL_1 | ADC12EOS; // A4 ADC input select; Vref=1.2V. Reference off for pin input.
  ...
  switch (__even_in_range(ADC12IV, ADC12IV_ADC12RDYIFG))
  {
    case ADC12IV_ADC12IFG0:                 // Vector 14:  ADC12MEM0 Interrupt
        ...
        if (ADC12MEM0 <= 5) { // when sensor reads full darkness
            ...
            TB0CCR6 = (ADC12MEM0 / 5); // set blue PWM
            P2OUT |= BIT1; P2OUT |= BIT0; // turn both LEDs on
        }
        if ((ADC12MEM0 > 5) && (ADC12MEM0 <= 35)) { // when sensor reads partial darkness
            TA1CCR1 = ADC12MEM0;
            TB0CCR5 = (ADC12MEM0 / 3);
            TB0CCR6 = (ADC12MEM0 / 5);
            P2OUT |= BIT1; P2OUT &= ~BIT0;
        }
